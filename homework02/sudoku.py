...
def get_block(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.List[str]:
    ...
    ind1 = pos[0] // 3 * 3
    ind2 = pos[1] // 3 * 3
    block = list(grid[ind1][ind2:ind2 + 3] + grid[ind1 + 1][ind2:ind2 + 3] + grid[ind1 + 2][ind2:ind2 + 3])
    return block
...
def find_possible_values(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.Set[str]:
    ...
    True
    """
    poss_val = {'1', '2', '3', '4', '5', '6', '7', '8', '9'}
    row_val = set(get_row(grid, pos))
    col_val = set(get_col(grid, pos))
    block_val = set(get_block(grid, pos))
    result = poss_val.difference(row_val, col_val, block_val)
    return result
